...
from virne.data.network import PhysicalNetwork, MultiDomainPhysicalNetwork
...
class MultiAgentScenario(Scenario):
    ...
    def __init__(self, env, solvers, config):
        super(MultiAgentScenario, self).__init__(env, solvers, config)
        self.agents = solvers
    ...
    def run(self):
        ...
        for epoch_id in range(self.config.num_epochs):
            ...
            while True:
                # TODO: 每个agent利用auction Solver 进行 auction
                bids = [agent.solve(instance) for agent in self.agents]
                self.env.place_bids(bids)

                winners = self.env.select_winner()

                # TODO: 从 winners 重组 instance

                # TODO: 每个agent利用自己的solver进行求解
                solutions = [agent.solve(instance) for agent in self.agents]

                # Assuming a method to combine solutions from multiple agents
                combined_solution = self.combine_solutions(solutions)

                next_instance, _, done, info = self.env.step(combined_solution)
    ...
    def combine_solutions(self, solutions):
        """
        Combine solutions from multiple agents into a single solution.

        Args:
            solutions (list): List of solutions from multiple agents.

        Returns:
            dict: Combined solution.
        """
        # Implement your logic to combine solutions from multiple agents
        combined_solution = solutions[0]  # Placeholder logic
    ...
    def from_config(cls, Env, Solver, config):
        ...
        print(config.p_net_dataset_dir)
        if os.path.exists(config.p_net_dataset_dir):
            p_net = MultiDomainPhysicalNetwork.load_dataset(config.p_net_dataset_dir)
            print(f'Load Physical Network from {config.p_net_dataset_dir}') if config.verbose >= 1 else None
        else:
            p_net = MultiDomainPhysicalNetwork.from_setting(config.p_net_setting)
            print(f'*** Generate Physical Network from setting')
        v_net_simulator = VirtualNetworkRequestSimulator.from_setting(config.v_sim_setting)
        print(f'Create VNR Simulator from setting') if config.verbose >= 1 else None
